...
public class Parser {
    public static LinkedList<String> parseAndExecute(String input
            , TaskList tasks, FileHandler fileHandler) throws DukeException {
        ...
        case "find":
            if (inputArray.length < 2) {
                throw new DukeException("OOPS!!! There must be a keyword for find.");
            }
            
            //Iterate through tasklist and Search for tasks with this keyword
            for (int i = 1; i <= tasks.getTaskListLength(); i++) {
                Task keyTask = tasks.getTask(i - 1);
                if (keyTask.getDescription().contains(inputArray[1])) {
                    result.add(i + ". " + keyTask);
                }
            }
